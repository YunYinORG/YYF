...
class Database extends PDO
{
    ...
    public function query($sql, array $params = null, $fetchAll = true, $mode = null)
    {
        if ($before = &self::$before) {
            ...
            call_user_func_array($before, [&$sql, &$params, 'query']);
        }
        ...
        if ($after = &self::$after) {
            ...
            call_user_func_array($after, [&$this, &$result, 'query']);
        }
    ...
    public function exec($sql, array $params = null)
    {
        ...
        if ($before = &self::$before) {
            //执行前调用
            assert('is_callable($before)', '[Database::$before] 应该是可执行的回调');
            call_user_func_array($before, [&$sql, &$params, 'exec']);
        }
        ...
        if ($after = &self::$after) {
            //执行完成调用
            assert('is_callable($after)', '[Database::$after] 应该是可执行的回调');
            call_user_func_array($after, [&$this, &$result, 'exec']);
        }
    ...
    public function column($sql, array $params = null)
    {
        if ($before = &self::$before) {
            //执行前调用
            assert('is_callable($before)', '[Database::$before] 应该是可执行的回调');
            call_user_func_array($before, [&$sql, &$params, 'column']);
        }
        ...
        if ($after = &self::$after) {
            //执行完成调用
            assert('is_callable($after)', '[Database::$after] 应该是可执行的回调');
            call_user_func_array($after, [&$this, &$result, 'column']);
        }
    ...
    private function error()
    {
        ...
        if ($after = &self::$after) {
            //执行完成调用
            assert('is_callable($after)', '[Database::$after] 应该是可执行的回调');
            call_user_func_array($after, [&$this, &$this->_errorInfo, 'error']);
        }
