...
class Orm implements \JsonSerializable, \ArrayAccess
{
    ...
    private $_param = []; //查询参数
    private $_joins = []; //join 表
    private $_groups = []; //group by
    private $_unions = []; //合并查询
    private $_fields = []; //查询字段
    private $_where = []; //查询条件
    private $_having = []; //having 条件
    private $_order = []; //排序字段
    ...
    public function put($key, $value)
    {
        if ($key = $this->getTrueField($key)) {
            $update = [$key => $this->bindParam($value)];
    ...
    {
        if (!in_array($type, ['INNER', 'LEFT', 'RIGHT', 'OUTER', 'FULL OUTER'])) {
            throw new Exception('[Orm::join] 第二个参数type不是JOIN支持的关联方式:'.$type, 1);
        }
        $fun_num = func_num_args();
        if (4 === $fun_num) {
            //快速设置JOIN ON条件
            $this->_joins[] = [$type, $table, $on, $related_key];
        } else {
            //高级设置
            ...
            foreach ($on as $key => &$value) {
                if (is_string($key)) {
                    //键值对
                    $conditions = $this->parseCondition([$key, $value], 'AND', false);
                } else {
                    //复杂逻辑
            ...
            $this->_joins[] = [$type, $table, $conditions];
        }
    ...
    protected function buildInsert(array &$data, $is_sqlite = false)
    {
        ...
        if ($is_sqlite) {
            ...
            foreach (current($data) as $key => $value) {
                $sql .= $value.' AS'.self::backQoute($key).',';
            }
    ...
    protected function buildSelect($exp = null)
    {
        ...
        if (null !== $exp) {
            ...
            $sql .= empty($this->_joins) ? '*' : self::backQoute($table).'.*';
        }
    ...
    protected function buildDelete()
    {
        if ($alias = &$this->_alias) {
            $sql = 'DELETE'.self::backQoute($alias);
        } else {
            $sql = 'DELETE ';
        }
        $sql .= $this->buildFrom()
    ...
    protected function buildWhere($check_pk = false)
    {
        if ($where = &$this->_where) {
            ...
            //see parseCondition()
            $where = [
                ['AND', $this->_pk, '=', $this->bindParam($this->_data[$this->_pk])],
            ];
    ...
    protected function buildTail($offset = true)
    {
        ...
        if (false === $offset) {
            ...
            if ('mysql' === $driver) {
                ...
                && !in_array(['compile_option' => 'ENABLE_UPDATE_DELETE_LIMIT'], $db->query('PRAGMA compile_options'))) {
    ...
    {
        if (true === $this->_debug) {
            return ['sql' => $sql, 'param' => $this->_param];
        } elseif ($this->_clear) {
    ...
    protected function parseFunction($str)
    {
        ...
        } elseif (in_array($fun, ['ABS', 'ASWKT', 'ASBINARY', 'ASTEXT', 'AVG', 'COUNT', 'LCASE', 'LENGTH', 'MAX', 'MIN', 'SIGN', 'SUM', 'UCASE'])) {
            $arg = trim(strtok(')')); //字段
        ...
        if (is_array($where[0])) {
            ...
            foreach ($where[0] as $key => $value) {
                ...
                $this->_where[] = is_array($value) ?
                     ...
                     $this->parseCondition([$key, $value], $type, $bind_value);
            }
        } else {
    ...
    protected function parseCondition(array $condition, $addition = null, $bind_value = true)
    {
        ...
        $result = $addition ? [$addition, $condition[0]] : [$condition[0]];
        switch (count($condition)) {
            ...
            case 4: //4元表达式between
                ...
                $result[] = $operator;
                $result[] = $bind_value ?
                    [$this->bindParam($condition[2]), $this->bindParam($condition[3])] :
